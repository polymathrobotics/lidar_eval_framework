...
class SpatialPointCount(MetricsBase):
    """Counts spatial (bounding-box-filtered) returns per cylindrical zone.

    Simple yield indicator: per scan it counts the finite points the spatial
    (3D box) filter kept for each zone, accumulates over the run, and reports the
    total, the mean per scan, and the scan count per zone.
    """
