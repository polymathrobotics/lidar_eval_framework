...
from lidar_transforms.tools.profiles_serializer import profiles_from_json
...
class LidarController(Node):
    ...
    def _on_profiles_response(self, future) -> None:
        ...
        profiles = profiles_from_json(response.profiles_json)
    ...
    def _on_filter_response(self, future):
        ...
        if not response.zone_names:
            self.get_logger().warn(f'ROI filter preflight failure: {response.message}')
            return
        ...
        total_spatial = sum(len(p) for p in spatial_by_zone.values())
        total_projective = sum(len(p) for p in projective_by_zone.values())
        self.get_logger().info(
            f'Spatial points: {total_spatial}, projective points: {total_projective} '
            f'across {len(response.zone_names)} zones'
        )
