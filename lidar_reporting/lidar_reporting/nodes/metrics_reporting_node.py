...
class MetricsReportingNode(Node):
    def __init__(self):
        super().__init__('metrics_reporting_node')
        ...
        self.get_logger().info('Authenticating Selected Database with Selected Authentication Method — follow the prompts in this terminal...')
        try:
            self.db_handler.authenticate()
            self.get_logger().info('Database Authentication successful')
        except Exception as e:
            self.get_logger().error(
                f'Database authentication failed: {e}. Drive sync and visualization push will be disabled.'
            )
    ...
    def _lidar_position(self, case_data: dict) -> np.ndarray | None:
        """Lidar xyz in the map frame, read from the report's `__global__` block (written by
        the metrics engine). Used to shift viz coords into the lidar frame — no live TF."""
        pos = case_data.get('__global__', {}).get('lidar_position')
        if isinstance(pos, (list, tuple)) and len(pos) >= 3:
            return np.array([float(pos[0]), float(pos[1]), float(pos[2])])
        return None

    def _transform_fitted_viz(self, viz: dict, t: np.ndarray) -> dict:
        result = dict(viz)
    ...
    def _fitted_plane_blocks(self) -> list:
        ...
        t = self._lidar_position(case_data)
        blocks = []
        ...
        for zone, metrics in case_data.items():
            ...
            for values in metrics.values():
                ...
                if t is not None:
                    prefixed = self._transform_fitted_viz(prefixed, t)
