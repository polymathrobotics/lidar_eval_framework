...
from lidar_transforms.tools.profile_builder import BaselineProfiles
from lidar_transforms.tools.transform_utils import apply_transform, build_transform_matrix
from lidar_transforms.tools.zones_utilities import PROJECTIVE_MASKS, SPATIAL_MASKS
...
class ROIFilter:
    ...
    def filter(
        ...
        projective_per_zone = self.projective_frustrum_filter(xyz, xyz_map, intensities, has_intensity, total_points, profiles, y_padding, z_padding)
    ...
    def spatial_filter(
        ...
        """
        per_zone_masks = self._roi_mask(xyz_map, profiles)
    ...
    def projective_frustrum_filter(
        ...
        """
        per_zone_masks = self._frustrum_mask(xyz_map, profiles, y_padding, z_padding)
        return self._build_per_zone_results(
    ...
    def _roi_mask(self, xyz_map: np.ndarray, profiles: BaselineProfiles) -> dict[str, np.ndarray]:
        """Per-zone boolean masks for the spatial filter.

        Routes each zone to its geometry-specific spatial mask via the
        SPATIAL_MASKS registry (planar bbox / cylindrical radial shell, both
        over the full extent). A point can appear in multiple zones if they
        overlap — no exclusivity is enforced.
        """
        return {
            zb.name: SPATIAL_MASKS.for_obj(zb)(zb, xyz_map)
            for zb in profiles.zone_bounds
        }
    ...
    def _frustrum_mask(
        ...
        PROJECTIVE_MASKS registry. The handlers apply their padding INWARD to
        tighten the cone. A point can appear in multiple zones if windows overlap.
        """
        lx, ly, lz = profiles.lidar_position
        dx = xyz_map[:, 0] - lx
        dy = xyz_map[:, 1] - ly
        dz = xyz_map[:, 2] - lz
        az = np.arctan2(dy, dx)
        ...
        return {
            zb.name: PROJECTIVE_MASKS.for_obj(zb)(
                zb, az, el, profiles.lidar_position, y_padding, z_padding
            )
            for zb in profiles.zone_bounds
        }
