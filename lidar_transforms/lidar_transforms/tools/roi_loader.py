...
# Zone data structs + the geometry-type parser registry live in zones_utilities.
# Re-exported here so existing `from roi_loader import ZoneConfig/ROIConfig` works.
from lidar_transforms.tools.zones_utilities import (  # noqa: F401  (re-exported)
    GEOMETRY_SURFACE,
    ROIConfig,
    ZONE_TYPE_PARSERS,
    ZoneConfig,
...
class ROILoader:
    ...
    def _parse_zone(self, raw: dict, index: int) -> ZoneConfig:
        ...
        geometry, and the geometry-specific fields are parsed by the registered
        handler in zones_utilities.ZONE_TYPE_PARSERS.
        """
        ...
        zone_type = ZONE_TYPE_PARSERS.resolve(geometry)(raw, location)
